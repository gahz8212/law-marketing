...
4. [DevOps & CI/CD 무중단 자동 배포 파이프라인 (CI/CD Pipeline)](#4-devops--cicd-무중단-자동-배포-파이프라인-cicd-pipeline)
5. [핵심 기능 및 사용자 인터랙션 (Core Features)](#5-핵심-기능-및-사용자-인터랙션-core-features)
6. [기술 스택 및 선정 이유 (Tech Stack & Why)](#6-기술-스택-및-선정-이유-tech-stack--why)
7. [핵심 RESTful API 명세서 (API Specification)](#7-핵심-restful-api-명세서-api-specification)
8. [기술적 도전과 트러블슈팅 (Technical Challenges)](#8-기술적-도전과-트러블슈팅-technical-challenges)
9. [정량적 비즈니스 임팩트 (Business Impact)](#9-정량적-비즈니스-임팩트-business-impact)
10. [회고 및 향후 로드맵 (Retrospective & Roadmap)](#10-회고-및-향후-로드맵-retrospective--roadmap)
...
| **진행 기간** | 2026.09.03 (1-Day 집중 스프린트 / 풀스택 & AI 엔지니어링 100%) |
| **담당 역할** | 풀스택 아키텍처 설계, AI 프롬프트 엔지니어링, TTS 음성 합성, AWS EC2 인프라 & GitHub Actions CI/CD 무중단 배포 |
...
| **인프라/DevOps** | AWS EC2 (Ubuntu 24.04), Nginx Reverse Proxy, Cloudflare Edge CDN, Let's Encrypt SSL, PM2 Zero-downtime, GitHub Actions CI/CD |
...
1. **난해한 판결문과 극심한 소요 시간:**
   - 대법원 판결문은 전문 법률 용어와 복잡한 사실관계로 가득 차 있어, 비법조인 마케터가 블로그 글 1편을 작성하는 데 **평균 3시간에서 4시간 소요**됨.
2. **높은 외주 콘텐츠 제작 비용:**
   - 인스타그램 카드뉴스 디자인 외주 건당 10만 원에서 15만 원, 숏츠 나레이션 성우 녹음 건당 10만 원 등 **판례 1건 홍보마다 약 20만 원에서 25만 원의 외주 비용 발생**.
3. **수동 멀티채널 배포의 비효율:**
...
## 4. DevOps & CI/CD 무중단 자동 배포 파이프라인 (CI/CD Pipeline)

본 프로젝트는 로컬 개발을 넘어 **실제 운영 중인 AWS EC2 클라우드 인프라에 프로덕션 배포**되었으며, 코드 변경 즉시 원클릭으로 빌드 및 무중단 배포가 수행되는 **GitHub Actions 기반 CI/CD 자동화 파이프라인**을 구축했습니다.

### (1) 엔드투엔드 배포 아키텍처 다이어그램 (Deployment Flow)
...
graph TD
    ...
    C["☁️ Cloudflare Edge CDN<br/>(SSL Full 프록시 & DDoS 방어)"]:::cloud

    subgraph AWS_EC2 ["AWS EC2 (Ubuntu 24.04 LTS)"]
        D["🌐 Nginx 리버스 프록시<br/>(SNI 기반 가상호스트 격리 & 443 SSL)"]:::ec2
        E["🚀 PM2 프로세스 매니저<br/>(24시간 데몬 상주 & Zero-Downtime)"]:::ec2
        F["🖥️ Next.js 16 (Port 3000)"]:::ec2
        G["⚙️ FastAPI (Port 8000)"]:::ec2
    end

    A ==>|git push origin main| B
    B ==>|SSH 원격 자동 실행| AWS_EC2
    C -->|HTTPS 라우팅| D
    D -->|/ (루트)| F
    D -->|/api/| G
    E -.->|무중단 프로세스 감시| F
...
### (2) GitHub Actions 워크플로우 명세 (`.github/workflows/deploy.yml`)
- **트리거 (Trigger):** `main` 브랜치에 `push` 이벤트 발생 시 자동 실행.
- **보안성 (Security):** `EC2_HOST`, `EC2_USER`, `EC2_SSH_KEY`를 GitHub Secrets로 비대칭 암호화 관리하여 소스코드 내 민감 정보 유출 0%.
- **배포 5단계 자동화 스크립트:**
  1. **소스코드 동기화:** `git fetch origin main && git reset --hard origin/main`
  2. **백엔드 의존성 갱신:** Python 3.12 가상환경 `.venv/bin/pip install -r requirements.txt`
  3. **프론트엔드 빌드:** Next.js 16 Turbopack 기반 프로덕션 번들링 (`npm run build`)
  4. **Nginx 설정 검증:** `nginx -t` 구문 검증 후 즉시 무중단 리로드 (`systemctl reload nginx`)
  5. **PM2 프로세스 핫 리로드:** 서비스 다운타임 없는 프로세스 갱신 (`pm2 restart ecosystem.config.js`)

---

## 5. 핵심 기능 및 사용자 인터랙션 (Core Features)
...
## 6. 기술 스택 및 선정 이유 (Tech Stack & Why)
...
| **DevOps** | **GitHub Actions, AWS EC2**<br>Nginx, PM2, Cloudflare, Let's Encrypt | 가상호스트 SNI 기반 레거시 격리, 무중단 자동화 CI/CD 및 SSL 종단 | Git Push 한 번으로 프로덕션 무중단 자동 빌드 및 기존 서비스 100% 무영향 공존 |

---

## 7. 핵심 RESTful API 명세서 (API Specification)
...
## 8. 기술적 도전과 트러블슈팅 (Technical Challenges)
...
- **결과 (Result):**
### 🔥 Issue 5. 기존 운영 중인 MES 시스템 무영향 공존 및 Nginx SNI 멀티 SSL 가상호스트 완벽 격리
- **문제 상황 (Situation):**
  - 단일 AWS EC2 인스턴스 내에 이미 24시간 가동 중인 레거시 서비스(`mes.memyself.shop`, Node-RED/PHP)가 포트 443 SSL을 선점하고 있어, 신규 서브도메인(`law.memyself.shop`) 접속 시 Nginx의 Default Server Fallback 동작에 의해 `mes`의 `/login.html`로 트래픽이 오라우팅(Misrouting)되는 현상 발생.
- **해결 과정 (Action):**
  1. 기존 `mes` 서비스의 프로세스와 포트(1880/FPM)를 분석하여 상호 포트 충돌이 없도록 신규 스튜디오의 포트를 3000(Next.js)과 8000(FastAPI)으로 명확히 할당.
  2. Nginx SNI(Server Name Indication) 기술을 활용하여 `law.memyself.shop` 전용 Let's Encrypt SSL/TLS 인증서를 독립 발급 및 장착.
  3. `sites-available/law.memyself.shop` 설정 내에 HTTP(80) ➔ HTTPS(443) 강제 리다이렉트 및 전용 443 SSL 블록을 명시적으로 구성하여 다른 가상 호스트와의 크로스 간섭 원천 차단.
  4. EBS 볼륨을 무중단 온라인 리사이징(8GB ➔ 20GB)하고 2GB 스왑 가상 메모리를 구축하여 Next.js Turbopack 빌드 시 발생하는 OOM(Out of Memory) 스로틀링을 원천 해소.
- **결과 (Result):**
  - **기존 MES 서비스의 가동률 100% 무영향(Zero-Impact) 유지**와 동시에, 신규 `law.memyself.shop` 프로덕션 서비스를 완벽하게 분리 격리 서빙 성공.

---

## 9. 정량적 비즈니스 임팩트 (Business Impact)
...
| **판례 1건당 외주 제작 비용** | 건당 **약 20만 - 25만 원** | **0원 (인하우스 AI 자동화)** | **💰 비용 100% 절감** |
| **원클릭 배포 채널 수** | 블로그 단일 채널 수동 등록 | 블로그 + 슬랙 + 노션 + 숏츠 MP3 | **🚀 채널 커버리지 4배 확장** |
| **인프라 CI/CD 배포 소요 시간** | 서버 수동 접속 및 빌드 **30분** | Git Push 자동화 **1분 20초** | **⚡ 95.5% 배포 효율화** |
| **AI 비전 상표권 리스크** | 대기업/언론사 로고 무단 합성 위험 | 100% 무상표 가상 그래픽 감사 완료 | **🛡️ 법적 분쟁 리스크 0건** |

---

## 10. 회고 및 향후 로드맵 (Retrospective & Roadmap)

### 💡 프로젝트 회고 (Engineering Insight)
- **"프롬프트 엔지니어링은 AI 프로덕트의 10%에 불과하며, 실무에서 마주하는 상표권 침해 검증, 브라우저 캐시 버스팅, 쾌적한 컷아웃 UX, 비동기 웹훅 파이프라인, 그리고 실제 운영 서버에서 기존 시스템과 충돌 없이 공존시키는 Nginx SNI 가상호스트 격리 및 GitHub Actions CI/CD 파이프라인 등 견고한 소프트웨어 엔지니어링이 뒷받침되어야 진정한 비즈니스 가치가 완성된다는 것을 체득했습니다."**
